  ...
  grunt.registerMultiTask('slack', 'Push info to slack', function() {
    ...
    var done = this.async(),
      ...
      url = options.endpoint ? options.endpoint :
        'https://' + options.domain + '.slack.com/services/hooks/incoming-webhook?token=' + options.token,
      data = this.data;
    ...
    request.post(url).type('form').send('payload=' + encodeURIComponent(JSON.stringify(data))).end(function(res) {
      if (!res.ok) {
